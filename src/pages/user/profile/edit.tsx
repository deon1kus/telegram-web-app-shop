...
function EditProfile() {
  ...
  return (
    <Container title="Редактировать Аккаунт" backwardUrl={-1}>
          ...
          <Form
            ...
            onFinish={(e) => {
              ...
              if (!phoneNumberValidator(e.phone_number)) {
                message.error("Введенный номер телефона неверен");
                form.scrollToField("phone_number");
              } else {
                mutation.mutate(e, {
                  onSuccess: () => {
                    message.success("Ваш аккаунт обновлен ");
                    navigate(-1);
                  },
                  onError: (e) => {
                    message.error("Проблема при обновлении аккаунта ");
                  }
                });
              }
            }}
